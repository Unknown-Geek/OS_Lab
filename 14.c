...
int lfu(int ref[LEN])
{
    ...
    for (int i = 0; i < LEN; i++)
    {
        if (!page_check(frames, ref[i]))
        {
            ...
            freq[lf_index] = 1; // Reset frequency for new page
            faults++;
        }
        else
        {
            // Increment frequency of the page that was hit
            for (int j = 0; j < FRAMES; j++)
            {
                if (frames[j] == ref[i])
                {
...
// Add main function to test the page replacement algorithms
int main()
{
    srand(time(NULL));
    ref_string();
